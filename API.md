...
Package main provides the entry point for the go-filemover utility. go-filemover
is a WSL2 native file watcher that moves files to a target directory (often on
the Windows host) based on glob patterns.
...
type Mover struct {
    ...
    Mover coordinates file operations using an abstracted FileSystem.

func NewMover(f FileSystem) *Mover
    NewMover creates a new Mover instance with the provided FileSystem.
...
func (m *Mover) MoveFile(src, dst string) error
    ...
    [FileSystem.Rename], and falls back to Mover.CopyAndDelete only if a
